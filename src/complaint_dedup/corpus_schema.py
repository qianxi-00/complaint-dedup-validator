...
work_orders = Table(
    ...
    Column("feature_json", JSON, comment="主体、地址、事项、发生对象等判重特征 JSON"),
    Column("feature_version", String(32), comment="判重特征规则版本"),
    ...
    Index("ix_work_orders_canonical_id", "canonical_work_order_id"),
    Index("ix_work_orders_complaint_fingerprint", "complaint_fingerprint"),
    Index("ix_work_orders_feature_version", "feature_version"),
...
comparison_runs = Table(
    ...
    Column("algorithm_version", String(64), nullable=False, server_default="event-key-v3", default="event-key-v3", comment="本次任务使用的判重算法版本"),
    Column("feature_version", String(32), nullable=False, server_default="feature-v2", default="feature-v2", comment="本次任务使用的判重特征版本"),
    Column("prompt_version", String(64), nullable=False, server_default="event-card-v1", default="event-card-v1", comment="事件卡提示词版本"),
    Column("model_id", String(255), comment="本次任务配置的事件卡模型标识"),
    Column("llm_coverage", Float, nullable=False, server_default="0", default=0, comment="进入事件卡模型裁决的工单覆盖率"),
    Column("fallback_count", Integer, nullable=False, server_default="0", default=0, comment="模型失败或未覆盖时的规则降级数"),
    Column("decision_count", Integer, nullable=False, server_default="0", default=0, comment="持久化的自动合并决策数"),
...
comparison_decisions = Table(
    "comparison_decisions",
    metadata,
    Column("id", Integer, primary_key=True, autoincrement=True, comment="判重决策主键"),
    Column("comparison_id", String(64), ForeignKey("comparison_runs.id", ondelete="CASCADE"), nullable=False, comment="所属比对任务 ID"),
    Column("card_id", String(128), nullable=False, comment="事件卡或规则组标识"),
    Column("card_ids", JSON, nullable=False, default=list, comment="参与本次决策的完整事件卡 ID 列表"),
    Column("assigned_card_ids", JSON, nullable=False, default=list, comment="模型代表卡分组后按规则分配的事件卡 ID 列表"),
    Column("decision", String(64), nullable=False, comment="决策类型：hard_merge、rule_merge、model_merge、model_assignment、rule_fallback 或 keep_separate"),
    Column("confidence", Float, nullable=False, default=0, comment="决策置信度"),
    Column("evidence_json", JSON, nullable=False, default=list, comment="支持合并的结构化证据"),
    Column("conflict_json", JSON, nullable=False, default=list, comment="阻止合并的结构化冲突"),
    Column("rule_version", String(64), nullable=False, comment="判重规则版本"),
    Column("prompt_version", String(64), comment="事件卡提示词版本"),
    Column("model_id", String(255), comment="实际调用的模型标识"),
    Column("final_event_key", String(1024), comment="该审计决策对应的最终事件键，跨多个事件时为空"),
    Column("fallback_reason", Text, comment="自动降级原因"),
    Column("elapsed_ms", Integer, comment="模型或规则决策耗时毫秒"),
    Column("record_keys", JSON, nullable=False, default=list, comment="本次决策涉及的工单稳定键"),
    Column("created_at", DateTime(timezone=True), nullable=False, comment="决策创建时间"),
    Index("ix_comparison_decisions_comparison", "comparison_id"),
    comment="任务内自动判重决策审计记录",
)
